...
assert(snippets.includes("button.setAttribute('aria-label', 'In Zwischenablage kopieren')"));
assert(snippets.includes('function makeEditorButton(snippet)'));
assert(snippets.includes("button.setAttribute('aria-label', 'Im Editor öffnen')"));
assert(snippets.includes('fa-file-import'));
assert(snippets.includes('footer.append(makeEditorButton(snippet), makeCopyButton(snippet))'));
assert(snippets.includes('function openSnippetInEditor(event, snippet)'));
assert(snippets.includes('openTargetDialog(snippet, result.suggestedTarget)'));
assert(snippets.includes("track('snippet_library_open_editor'"));
assert(snippets.includes("window.showToast?.('Baustein kopiert', 'success')"));
...
assert(snippets.includes('await window.copyTextToClipboard(content)'));
