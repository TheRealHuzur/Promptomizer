...
assert(library.includes('await navigator.clipboard.writeText(text)'));
assert(library.includes("track('library_prompt_copy')"));
assert(!library.includes("await window.handlePromptClick?.(id)"));
...
assert(css.includes('#view-editor.prompt-edit-active {'));
