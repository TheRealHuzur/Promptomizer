...
| `library.css` | Desktop-Design der Prompt-Bibliothek; bewusst ohne appweites Responsive-Redesign |
| `snippet-library.js` | Bausteinbereich der zentralen Bibliothek: Suche, Karten/Liste, Filter, Bearbeitung, Archiv, Export und Pro-Massenaktionen |
| `snippet-library.css` | Ergänzende Styles für Bereichsumschalter, Bausteinbearbeitung und Zielauswahl |
| `kollektionen.js` | Statische, öffentliche Kollektionsdaten (`window.PROMPTOMIZER_COLLECTIONS`), Pflege per Git; stabile `id`s |
| `collections.js` / `collections.css` | Ansicht „Kollektionen“: Vorschau mit Platzhalter-Hervorhebung, Kopieren (frei) und Übernahme in die eigene Bibliothek |
...
- **Gemeinsames Free-Limit (Phase 7, produktiv):** Trigger auf `library` und `snippets` verwenden dieselbe Funktion `check_free_plan_limit`, denselben nutzerbezogenen Advisory Lock und zählen Prompts plus Bausteine einschließlich Archiv. Ab zehn Inhalten blockieren Inserts und Duplikate mit `FREE_LIMIT_REACHED`; Bearbeiten, Exportieren, Archivieren und Löschen bleiben auch für Bestandsnutzer oberhalb der Grenze möglich. Das Frontend zählt entsprechend beide Tabellen.
...
- **Baustein-Bibliothek (Phase 7, produktiv):** `snippets` erhält `updated_at`, `is_favorite`, `last_used_at`, `archived_at` und eine generierte Volltext-`search_vector`. `mode`/`field_id` werden konsistent auf fünf Einsatzbereiche begrenzt; RLS nutzt `TO authenticated`, Eigentümerprüfung sowie `USING`/`WITH CHECK`. Duplikate, Zähler und Pro-Mehrfachaktionen laufen über eigentümergebundene `SECURITY INVOKER`-Funktionen.
...
- **Bibliotheks-Bearbeitung:** `promptEditSession` lädt einen Prompt bewusst in den Haupteditor und sperrt den Moduswechsel. „Zur Bibliothek“ beendet die verknüpfte Bearbeitung, stellt den vorherigen Editorentwurf wieder her und öffnet die Bibliothek. Ein Klick auf den Sidebar-Eintrag „Editor“ beendet eine noch aktive verknüpfte Bearbeitung ebenfalls und öffnet den normalen Editor. Ein erfolgreiches `saveActivePromptVersion()` beendet dagegen die Bearbeitung und lässt den aktualisierten Prompt im normalen Editor stehen. Normales `handlePromptClick()` bleibt für andere Oberflächen ein nicht verknüpftes Laden als Vorlage; der Kopierbutton auf Bibliothekskarten und -listen schreibt dagegen `promptToText()` direkt in die Zwischenablage, aktualisiert `last_used_at` und lässt die Bibliothek geöffnet. Beim Bearbeiten eines Prompts zeigt der Hauptfooter nur „Neue Version speichern“ und „Kopieren“, nicht Reset. In der normalen `view-library` zeigt er kontextabhängig „Neuer Prompt“ oder „Neuer Baustein“. Die zentrale Suche erzeugt pro Suchwort eine PostgreSQL-Präfixabfrage (`:*`), damit beispielsweise „Master“ auch „Masterplan“ findet.
...
- **Bausteine/Snippets (Phase 7 produktiv ausgerollt):** Das Sidebar-Accordion (`renderSnippetsAccordion`, `toggleSnippetSection`, `loadSnippetSection`, `openSnippetCategory`, `insertSnippetEncoded`/`insertSnippetText`) bleibt optisch und bedienseitig unverändert. `snippet-library.js` ergänzt in `view-library` den getrennten Bereich „Bausteine“ mit Suche, Karten/Liste, Verwaltung, Archiv und Export. Sidebar und zentrale Bibliothek führen auf `insertSnippetText`; nur die zentrale Bibliothek zeigt für freie Bausteine im strukturierten Editor eine Zielauswahl. `last_used_at` wird erst nach erfolgreichem Einfügen oder Kopieren geschrieben. Beim Bearbeiten eines bestehenden Bausteins übernimmt der Hauptfooter die Aktionen „Neue Version speichern“ und „Kopieren“ ohne Reset; die Speicheraktion aktualisiert weiterhin den vorhandenen Baustein, da es keine eigene Baustein-Versionierung gibt. Ältere `loadSnippetsForField*`/`insertSnippet`-Funktionen bleiben entfernt — **nicht wieder einführen**.
- **Kollektionen:** `collections.js` hängt sich wie `library.js` per Nav-Eintrag, View und weiterem `switchView`-Wrapper ein und muss **nach** `library.js` geladen werden. Daten liegen ausschließlich in `kollektionen.js` (keine DB, keine RLS). Kopieren nutzt `window.copyTextToClipboard`, braucht keine Anmeldung und legt nichts an. „In meine Bibliothek übernehmen“ ist eine unabhängige Kopie über `db.saveScenario` im bestehenden Speicherformat (Vorprüfung und `FREE_LIMIT_REACHED` → `openUpgradeModal('library_full')`, Gäste → `openAuthModal({ hint })`). `openAuthModal`/`openRegisterModal` nehmen optional `{ hint }` für einen Anlass-Hinweis im Modal (`setAuthContextHint`, bleibt beim Wechsel Login ↔ Registrierung erhalten, Aufruf ohne Argument = kein Hinweis). Platzhalter `[...]` werden nur in der Vorschau per DOM-Spans hervorgehoben; kopierter und gespeicherter Text bleibt unverändert.
