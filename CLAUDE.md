...
- **Bausteine/Snippets:** laufen über das **Accordion** (`renderSnippetsAccordion`, `toggleSnippetSection`, `loadSnippetSection`, `openSnippetCategory`, `insertSnippetEncoded`/`insertSnippetText`). Sie sind noch nicht in die zentrale `view-library` integriert; dafür ist zunächst ein eigenes UX-/Umsetzungskonzept vorgesehen. Ältere `loadSnippetsForField*`/`insertSnippet`-Funktionen wurden als toter Code entfernt — **nicht wieder einführen**.
